...
from .cache import InfCache, LRUCacheDict, SLRUCache, TwoQueue, ARCCache, TwoCache
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        # code for LUT
        cache_size = _Program.cache_size
        print("Cache type:",type(_Program.lookup_table).__name__)
        print(f"Cache size: {cache_size}")
        if type(_Program.lookup_table).__name__ in ("SLRUCache", "SLRUCacheOrder", "TwoQueue"):
            print(f"Ratio: {_Program.lookup_table.ratio}")
        ...
        for gen in range(prior_generations, self.generations):
            ...
            # # Test code, clear cache every n generations
            # if (gen + 1) % 5 == 0:
            #     _Program.lookup_table.clear()
            
            # # Test code for InfLUT 无限缓存测试代码，记得测试完注释掉！
            # if type(_Program.lookup_table).__name__ == 'InfCache':
            #     print(f"Gen {gen} LUT contains {len(_Program.lookup_table)} key-value pairs")
        ...
        # # Test code for InfLUT 无限缓存测试代码，记得测试完注释掉！
        # if type(_Program.lookup_table).__name__ == 'InfCache':
        #     def print_program(key):# Test function is used to print program
        #         readable_program = []
        #         for node in key:
        #             if isinstance(node, _Function):
        #                 # Get the name of the function
        #                 readable_program.append(node.name)
        #             elif isinstance(node, int):
        #                 readable_program.append(f'x{node}')
        #             else:
        #                 # Keep other types unchanged
        #                 readable_program.append(node)
        #         return readable_program

        #     sorted_items = sorted(_Program.lookup_table.cache.items(), key=lambda item: item[1][1], reverse=True)
        #     top_100_items = sorted_items[:100]
        #     readable_top_100 = [(print_program(key), freq) for key, (result, freq) in top_100_items]
        #     print("Top 100 most frequent programs and their frequencies:")
        #     for idx, (program, freq) in enumerate(readable_top_100, start=1):
        #         print(f"{idx}: freq = {freq}, program = {program}")
        #     total_call = sum(item[1] for item in _Program.lookup_table.cache.values())
        #     print(f"LUT was called a total of {total_call} times.")
        #     target=total_call * 0.9
        #     current_sum = 0
        #     count = 0
        #     for item in sorted_items:
        #         current_sum += item[1][1]
        #         count += 1
        #         if current_sum >= target:
        #             break
        #     print(f"The top {count} items' call count reached 90% of the total call, accounting for {count * 100 / len(sorted_items):.2f}% of LUT size")
        #     current_sum = 0
        #     for item in sorted_items[:5000]:
        #         current_sum += item[1][1]
        #     print(f"The top 5k items' call count reached {current_sum}, accounting for {current_sum * 100 / total_call :.2f}% of total call")


        # Clear the cache after fit
        _Program.lookup_table.clear()
...
class SymbolicRegressor(BaseSymbolic, RegressorMixin):
    ...
    def __init__(self,
        ...
        _Program.cache_size = cache_size
        if cache_type.upper() == 'FIFO':
            _Program.lookup_table = cachetools.FIFOCache(maxsize=cache_size)
        elif cache_type.upper() == 'LFU':
            _Program.lookup_table = cachetools.LFUCache(maxsize=cache_size)
        elif cache_type.upper() == 'RR':
            _Program.lookup_table = cachetools.RRCache(maxsize=cache_size)
        elif cache_type.upper() == 'LRU':
            _Program.lookup_table = cachetools.LRUCache(maxsize=cache_size)
        # elif cache_type.upper() == 'LRUORDER':
        #    _Program.lookup_table = LRUCacheOrder(cache_size=cache_size)
        elif cache_type.upper() == 'LRUDICT':
            _Program.lookup_table = LRUCacheDict(cache_size=cache_size)
        elif cache_type.upper() == 'INF':
            _Program.lookup_table = InfCache()
        elif cache_type.upper() == 'SLRU':
            _Program.lookup_table = SLRUCache(cache_size=cache_size, probationary_ratio=ratio)
        # elif cache_type.upper() == 'SLRUSELF':
        #     _Program.lookup_table = SLRUCacheOrderSelf(cache_size=cache_size, probationary_ratio=ratio)
        # elif cache_type.upper() == 'SLRUORDER':
        #     _Program.lookup_table = SLRUCacheOrder(cache_size=cache_size, probationary_ratio=ratio)
        elif cache_type.upper() == 'TINYLFU':
            _Program.lookup_table = TinyLFU(size=cache_size, sample=cache_size/10, false_positive=0.01)
        elif cache_type.upper() == '2Q':
            _Program.lookup_table = TwoQueue(cache_size=cache_size, ratio=ratio)
        elif cache_type.upper() == 'ARC':
            _Program.lookup_table = ARCCache(cache_size=cache_size)
        elif cache_type.upper() == '2CACHE':
            _Program.lookup_table = TwoCache(cache_size=cache_size, ratio=ratio, complexity=complexity)
        ...
        if not isinstance(cache_size, int):
            raise ValueError(f"cache_size must be an integer, got {type(cache_size).__name__}")
        if not (0 < ratio < 1):
    ...
    def predict(self, X):
        ...
        """
        # Clear the cache to avoid calling the calculation results during training
        _Program.lookup_table.clear()
