...
from .cache import InfCache, LRUCacheDict, SLRUCache
...
class _Program(object):
    ...
    def get_subtree_as_tuple(self, idx):
        """
        获取从给定索引开始的完整子树，并将其转换为元组形式(作为字典的键)。
        Get the complete subtree starting at the given index and 
        convert it into a tuple form (to be used as a dictionary key).

        Parameters
        ----------
        idx : int
            开始子树的函数节点索引。
            Function node index at which to start the subtree.

        Returns
        -------
        subtree_tuple : tuple
            从给定索引开始的子树，表示为元组形式。
            The subtree starting at the given index, 
            represented as a tuple.

        """
        subtree = []
        stack = 1  # Initialize the stack to indicate the number of nodes to be found
        while stack > 0:
            node = self.program[idx]
            subtree.append(node)
            stack += node.arity - 1 if isinstance(node, _Function) else -1
            idx += 1
        # Convert the subtree into a tuple
    ...
    def execute(self, X):
        ...
        while idx < len(self.program):
            ...
            if isinstance(node, _Function):
                if idx == 0:
                    key = tuple(self.program)
                else:
                    key = self.get_subtree_as_tuple(idx)

                if key in _Program.lookup_table:
                    if apply_stack:
                        apply_stack[-1].append(_Program.lookup_table[key])   
                    else:
                        return _Program.lookup_table[key]
                    # skip the whole subtree
                    idx += len(key) - 1
                else:
                    apply_stack.append([node,key])
            else:
                # Lazily evaluate later
                apply_stack[-1].append(node)

            while len(apply_stack[-1]) == apply_stack[-1][0].arity + 2:
                ...
                _Program.lookup_table[key] = intermediate_result
