...
def hash_mm3_hash64(item, optimal_k, optimal_m):
    ...
    for i in range(optimal_k):
        ...
        yield bit_index
#loom Filter 的哈希策略。计算 optimal_k 个哈希值，将 item 映射到 optimal_m 个 bit 位 之一。

class BloomFilter: # 布隆过滤器类
    ...
    def insert(self, other: object):
        ...
        for i in self.probe_function(other, self.optimal_k, self.optimal_m):#这里有改动
            self.bits[i] = True
...
class Doorkeeper: # 封装 BloomFilter
